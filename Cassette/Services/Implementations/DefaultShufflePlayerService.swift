...
/// Thin PlayerServiceProtocol decorator that makes a newly-started song queue default to Shuffle.
///
/// The underlying PlayerService still owns all queue mutation, persistence, audio and playback state.
/// This decorator only changes the mode after an explicit `play(tracks:startIndex:)` succeeds, so:
/// - the song the user tapped starts first;
/// - the remaining queue is shuffled by PlayerService's existing shuffle implementation;
/// - internal next/previous, resume, restored sessions, radio and Smart Shuffle keep their existing behavior;
/// - users can still switch the active queue back to List or Repeat One afterwards.
actor DefaultShufflePlayerService: PlayerServiceProtocol {
    ...
    func setAutoExtendEnabled(_ enabled: Bool) async {
        await base.setAutoExtendEnabled(enabled)
    }
    ...
    func togglePlayPause() async {
        await base.togglePlayPause()
    }

    func saveCurrentPosition() async {
        await base.saveCurrentPosition()
    }

    func replayGainSettingsDidChange() async {
        await base.replayGainSettingsDidChange()
    }

    func crossfadeSettingsDidChange() async {
        await base.crossfadeSettingsDidChange()
    }

    nonisolated func stopAudioEngineSync() {
        base.stopAudioEngineSync()
    }
}
